...
This file provides scoped notation for writing concrete n-qubit Pauli group
elements the way the physics literature does:
...
The letters between the brackets form a single identifier over the alphabet `X`,
`Y`, `Z`, `I`, read left to right as qubits `0, 1, …`; the number of letters
fixes the number of qubits, so `σ[XIZ] : NQubitPauliGroupElement 3` and
`P[XIZ] : NQubitPauliOperator 3` with no ascription needed.

Parametric families, whose qubit count and positions are terms rather than
numerals, use the **symbolic form** with the same leading tokens:
...
where `n` and the indices are arbitrary terms and the letters are `X`, `Y`, `Z`
(see § "Symbolic (parametric) form" below).

The notation is **scoped**: enable it with `open scoped Pauli`. It must be
opt-in because any notation whose leading token is `σ[` takes over that token
from `GetElem` indexing (`σ[i]`) for variables named `σ` (likewise `iσ[…]` for
variables named `iσ`, and `P[…]` for variables named `P`). The phase prefixes
are part of the leading token (`-σ[`, `iσ[`, `-iσ[`): write them with no space
before `σ[`, and keep spaces around a binary `-` (`a - σ[XX]`) when you mean
subtraction with the scope open.

## Elaboration guarantee

`σ[…]` elaborates to **exactly** the canonical literal normal form used
throughout the concrete code files:

```lean
⟨phase, ((NQubitPauliOperator.identity n).set i₀ op₀).set i₁ op₁ ⋯⟩
```

with `.set` applied only at the non-identity positions, in increasing index
order, and `P[…]` elaborates to exactly the operator half of that form, the bare
`.set` chain (so `P[II…I]` is `NQubitPauliOperator.identity n` itself). The
notation is a macro (pure syntax expansion into that form), so kernel-reduction
behavior and every existing `simp`/`decide`/`rfl` proof over such literals are
unchanged. In particular there is deliberately no `ofList`-style helper on the
elaboration path: a list lookup is an O(n) walk under kernel reduction (see the
axiom-policy notes in `CLAUDE.md`), whereas the `.set` chain is the form all
existing proofs already consume.

## Delaborator

Terms of exactly that literal shape (literal phase, literal indices,
`PauliOperator` constructors, literal qubit count) display back as
`σ[…]`/`iσ[…]`/`-σ[…]`/`-iσ[…]` in goals and diagnostics, and a literal-shaped
`.set` chain on its own displays as `P[…]` (so an element whose phase is not a
literal shows as `⟨k, P[…]⟩`; a bare `identity n` keeps its name). The letters
form is used only for a chain in the literal normal form — literal indices in
strictly increasing order, no explicit `I` — so that the display re-parses to
the very same term. Any other `.set` chain over an `identity n` whose operators
are `X`/`Y`/`Z` constructors — symbolic qubit count or indices, literal indices
out of order or out of range — displays in the symbolic form `σ[n | i ↦ Z, …]` /
`P[n | i ↦ Z, …]`, with the `.set`s listed innermost first (the written order).
Anything else — variable phase or operators, an explicit `I`, a chain not rooted
at `identity` — is left to the default printer. The delaborators are `scoped`
with the notation, so a goal only ever shows syntax the current file can parse;
...
/-- `σ[XZIX]` is the `NQubitPauliGroupElement` with phase `+1`
(`phasePower = 0`) whose operator at qubit `k` is the `k`-th letter (letters
`X`, `Y`, `Z`, `I`; qubit count = letter count). Scoped: enable with
`open scoped Pauli`. Elaborates to the literal
`⟨0, (NQubitPauliOperator.identity n).set i₀ op₀ …⟩` normal form. -/
scoped syntax:max (name := sigma) "σ[" ident "]" : term

/-- `iσ[XZIX]`: as `σ[XZIX]` but with phase `i` (`phasePower = 1`). Scoped:
enable with `open scoped Pauli`. -/
scoped syntax:max (name := iSigma) "iσ[" ident "]" : term

/-- `-σ[XZIX]`: as `σ[XZIX]` but with phase `-1` (`phasePower = 2`). Scoped:
enable with `open scoped Pauli`. -/
scoped syntax:max (name := negSigma) "-σ[" ident "]" : term

/-- `-iσ[XZIX]`: as `σ[XZIX]` but with phase `-i` (`phasePower = 3`). Scoped:
enable with `open scoped Pauli`. -/
scoped syntax:max (name := negISigma) "-iσ[" ident "]" : term

/-- `P[XZIX]` is the bare Pauli string, an `NQubitPauliOperator` with no phase,
whose operator at qubit `k` is the `k`-th letter (letters `X`, `Y`, `Z`, `I`;
qubit count = letter count); it is `σ[XZIX].operators`. Scoped: enable with
`open scoped Pauli`. Elaborates to the literal
`(NQubitPauliOperator.identity n).set i₀ op₀ …` normal form. -/
scoped syntax:max (name := pauliString) "P[" ident "]" : term

/-- Split the letters identifier of a `σ[…]`-family literal into its characters,
checking that the identifier is a single atomic name over the alphabet `X`, `Y`,
`Z`, `I`. -/
...
/-- Expand the letters of a literal into the canonical operator-string normal
form `(NQubitPauliOperator.identity n).set i₀ op₀ …`, setting only the
non-identity positions, in increasing index order. This is the expansion of
`P[…]`, and the operator half of the `σ[…]` family. -/
...
/-- Expand a `σ[…]`-family literal with the given `phasePower` into the
...
`σ[n | i ↦ Z, j ↦ Z]` is the same literal with the qubit count `n` and the
indices `i`, `j` arbitrary terms — the form parametric families need, where the
qubit count is `numQubits L` or `n + 2` and the positions are `hEdge L x y` or
`Fin.succ i`. It expands to exactly

```lean
⟨0, ((NQubitPauliOperator.identity n).set i PauliOperator.Z).set j PauliOperator.Z⟩
```

with the `.set`s applied **in the written order** (there is no index to sort
by), so a downstream
`simp [NQubitPauliOperator.set, NQubitPauliOperator.identity]` sees the same
chain it always did. The letters are `X`, `Y`, `Z` (an explicit `I` is rejected:
leave the qubit out instead). The phase prefixes and `P[n | …]` work the same
way.
-/

/-- One qubit assignment `i ↦ Z` in a symbolic `σ[n | …]`-family literal: the
index `i` is an arbitrary term, the letter is `X`, `Y`, or `Z`. -/
syntax pauliAssign := term " ↦ " ident

/-- `σ[n | i ↦ Z, j ↦ Z]` is the `NQubitPauliGroupElement n` with phase `+1`
whose operator at `i` is `Z`, at `j` is `Z`, and `I` elsewhere, for arbitrary
terms `n`, `i`, `j` (letters `X`, `Y`, `Z`). Scoped: enable with
`open scoped Pauli`. Elaborates to the literal
`⟨0, ((NQubitPauliOperator.identity n).set i Z).set j Z⟩`, the `.set`s in the
written order. -/
scoped syntax:max (name := sigmaSym) "σ[" term " | " pauliAssign,+ "]" : term

/-- `iσ[n | i ↦ Z, …]`: as `σ[n | i ↦ Z, …]` but with phase `i`
(`phasePower = 1`). Scoped: enable with `open scoped Pauli`. -/
scoped syntax:max (name := iSigmaSym) "iσ[" term " | " pauliAssign,+ "]" : term

/-- `-σ[n | i ↦ Z, …]`: as `σ[n | i ↦ Z, …]` but with phase `-1`
(`phasePower = 2`). Scoped: enable with `open scoped Pauli`. -/
scoped syntax:max (name := negSigmaSym) "-σ[" term " | " pauliAssign,+ "]" : term

/-- `-iσ[n | i ↦ Z, …]`: as `σ[n | i ↦ Z, …]` but with phase `-i`
(`phasePower = 3`). Scoped: enable with `open scoped Pauli`. -/
scoped syntax:max (name := negISigmaSym) "-iσ[" term " | " pauliAssign,+ "]" : term

/-- `P[n | i ↦ Z, j ↦ Z]` is the bare Pauli string on `n` qubits with `Z` at `i`
and at `j`, for arbitrary terms `n`, `i`, `j`; it is
`σ[n | i ↦ Z, j ↦ Z].operators`. Scoped: enable with `open scoped Pauli`.
Elaborates to the literal `((NQubitPauliOperator.identity n).set i Z).set j Z`,
the `.set`s in the written order. -/
scoped syntax:max (name := pauliStringSym) "P[" term " | " pauliAssign,+ "]" : term

/-- The `PauliOperator` constructor named by a letter identifier `X`, `Y`, or
`Z`. -/
...
/-- Expand the symbolic assignments `i₀ ↦ op₀, i₁ ↦ op₁, …` on `n` qubits into
the chain `((NQubitPauliOperator.identity n).set i₀ op₀).set i₁ op₁ …`, in the
written order. This is the expansion of `P[n | …]`, and the operator half of the
`σ[n | …]` family. -/
...
Displays literal-shaped `NQubitPauliGroupElement.mk` applications back as
`σ[…]`, and literal-shaped `NQubitPauliOperator.set` chains back as `P[…]`; any
other `.set` chain over an `identity n` whose operators are `X`/`Y`/`Z`
constructors displays in the symbolic form `σ[n | i ↦ Z, …]` /
`P[n | i ↦ Z, …]`.
-/
...
/-- Match a literal natural number: a raw `Nat` literal or an `OfNat.ofNat`
application of one (the form numerals elaborate to). -/
...
/-- Match a literal `set`-chain `(NQubitPauliOperator.identity n).set i₀ op₀ …`
with literal indices and constructor operators. Returns the literal `n` together
with the `(index, letter)` assignments, innermost first. Returns `none` on any
other shape. -/
...
/-- The letters identifier of a literal chain over `n > 0` qubits whose indices
are all in range, or `none` otherwise. Later (outer) `.set`s override earlier
ones, so the innermost-first assignment list is folded left to right. -/
...
/-- The letters identifier of the `set`-chain `e`, if it is in the literal
normal form the letters notation expands to: literal `n > 0`, literal in-range
indices in strictly increasing order, and no explicit `I`. A chain of any other
shape (an out-of-order literal index, a set `I`) is left to the symbolic
printer, so that the display always re-parses to the same term. -/
...
`((NQubitPauliOperator.identity n).set i₀ op₀).set i₁ op₁ …` whose operators are
`X`/`Y`/`Z` constructors, with `n` and the indices arbitrary. Returns the
delaborated `n` and the delaborated indices with their letters, innermost first
(the written order of the notation); each subterm is delaborated at its own
position, so hover and `pp.` options behave as usual. Fails on any other shape,
including an explicit `I`. -/
...
`0`–`3` and the operator part is a `set`-chain over `identity n`: a chain in the
literal normal form (see `literalLetters?`) displays in the letters form
`σ[XIZ]`, any other chain with `X`/`Y`/`Z` operators in the symbolic form
`σ[n | i ↦ Z, …]`; stays silent otherwise. Scoped with the notation. -/
...
/-- Delaborate `NQubitPauliOperator.set` chains back to the `P[…]` notation.
Fires only on a `set` application whose whole chain runs over an `identity n`: a
chain in the literal normal form (see `literalLetters?`) displays as `P[XIZ]`,
any other chain with `X`/`Y`/`Z` operators as `P[n | i ↦ Z, …]`; stays silent
otherwise (in particular a bare `identity n` keeps its name). An over-applied
chain — the operator string evaluated at a qubit, `P[XIZ] i` — is handled by
`withOverApp`, so the extra arguments follow the notation. Scoped with the
notation. -/
...
and `P[…]` to exactly its operator half.
-/
...
/-! The symbolic form expands to the same chain with the `.set`s in the written
order. -/
...
The printers are checked on their output: literal normal forms print in the
letters form, everything else that re-parses prints in the symbolic form, and a
chain that would not re-parse to itself (an explicit `I`) is left to the default
printer. -/
