...
type `Fin n → PauliOperator`, see `Representation.lean`). From it one can derive
`DecidableEq (NQubitPauliGroupElement n)` by field-wise decision and then
`Decidable (Anticommute p q)` (which unfolds to an equality of two group
elements); the latter is necessarily `noncomputable`, because the `Mul` instance
on `NQubitPauliGroupElement` is, but the kernel still reduces `decide` through
it (`native_decide` does not work for the same reason — prefer `decide`).

Neither instance is declared here, on purpose. They were once added globally and
disrupted typeclass synthesis in an unrelated `native_decide` proof
(`RotatedSurfaceCode3`, since parked on `claude/z3z6-parked`) — the footgun
recorded under "Global vs. `local instance` discipline" in `CLAUDE.md`. They now
live as `local instance`s in `Codes/Small/FiveQubit_5_1_3.lean` (§ "Local
Decidable instances"), where the [[5,1,3]] distance proof needs them; copy them
from there into any other file that wants `decide` on `Anticommute`. -/
