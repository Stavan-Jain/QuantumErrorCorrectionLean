...
The check matrix of a list of n-qubit Pauli group elements has one row per
element: row i is the symplectic vector (Fin (n + n) → ZMod 2) of the i-th
element's operator part. Rows are linearly independent over ZMod 2 iff the
...
/-- The check matrix of a list of n-qubit Pauli group elements.
  ...
  operator part. Column indices are Fin (n + n): first n columns are
  X-components, last n are Z-components. -/
...
/-- The rows of the check matrix are linearly independent over ZMod 2.
  ...
  This is the predicate used (together with the equivalence theorem, to be
  proved later) to deduce that the generators form an independent set: no
  generator lies in the subgroup generated by the others. -/
