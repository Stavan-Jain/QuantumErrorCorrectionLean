...
logical. The **induced outer operator** `inducedOuter D g` records, per block,
the inner logical *class* of that restriction — read off from how it commutes
with the inner logicals `X̄₁`, `Z̄₁`:
...
* `inducedOuter_support_eq` — block `b` is in the support iff
  `restrictBlock b g` is a nontrivial inner logical (uses M4);
* `inducedOuter_mem_centralizer` — `inducedOuter D g` centralizes the outer
  stabilizer.

(`inducedOuter_not_mem_stabilizer` / `inducedOuter_isNontrivialLogical`, the
coset injectivity of plan risk R7, are developed separately.)
-/
...
open Classical in
/-- The inner logical class of block `b`'s restriction, read off from its
commutation with the inner logicals: `I` if it commutes with both, `X`/`Z` for a
single anticommutation, `Y` for both. -/
...
/-- The induced class at `b` is `I` exactly when the restriction commutes with
both inner logicals. -/
...
/-- **(Per-block bridge.)** For a `Y`-free entry `h.operators b`, the induced
operator anticommutes with `h` at block `b` exactly when the restriction
anticommutes with the promoted single-qubit Pauli
`ofOperator (promoteSingle … (h.operators b))` (= `1`, `X̄₁`, or `Z̄₁`). The
single-block analogue of `cnt_odd_iff`. -/
...
/-- Per-position reduction for the induced bridge: `g` anticommutes with the
promoted `h` at `q` exactly when the restriction anticommutes with the
single-qubit promotion of `h.operators b` at `posOf q` (within block
`b = blockOf q`). -/
...
open Classical in
/-- Block-decomposition: the anticommuting-position count of `g` against a
promoted outer operator is the sum over blocks of the per-block
restriction-vs-promotion counts. The mixed analogue of `promote_count_eq_sum`.
-/
...
open Classical in
/-- **(Induced parity bridge.)** For a `Y`-free outer operator `h`, the
anticommuting-position count of the induced operator against `h` has the same
parity as that of `g` against the promoted `h`. The dual of
...
/-- **(M5.)** For a `Y`-free outer operator `h`, the induced operator commutes
with `h` iff `g` commutes with the promoted `h`. -/
...
/-- **(M5.)** The induced outer operator centralizes the outer stabilizer: for
each outer generator `y`, `inducedOuter D g` commutes with `y` because `g`
commutes with `promoteE y` (a concatenated generator) and the induced bridge
transports that commutation. -/
...
/-! ## Support of the induced operator: nontrivial-block characterisation (uses
M4) -/
...
/-- **(Uses M4.)** A block restriction commutes with both inner logicals iff its
operator part is realised by an inner stabilizer. Forward is M4's decisive
kernel; backward is operator-part commutation (the restriction and the matching
stabilizer share an operator part, and the logicals centralise the stabilizer).
-/
...
/-- **(M5.)** Block `b` is in the support of the induced outer operator exactly
when the block restriction `restrictBlock b g` is a *nontrivial* inner logical.
This is what turns the per-block weight bound (`weight_ge_of_blocks_ge`) into a
count of nontrivial blocks. Uses M4 (`centralizer_classify_of_k1` and the
decisive kernel via `restrict_commutes_both_iff_stab`). -/
...
The nontriviality of `inducedOuter D g` rests on the symplectic fact
`inducedOuter_symp_in_span` below: if some outer stabilizer `t` matches the
operator part of `inducedOuter D g`, then `g`'s symplectic vector already lies
in the span of the concatenated check matrix — i.e. `g` is
concatenated-stabilizer-like. Contrapositively, a *nontrivial* concatenated
logical `g` cannot have a stabilizer-like induced operator, so
`inducedOuter D g` is itself a nontrivial outer logical.

`inducedOuter_symp_in_span` is now fully assembled from the symplectic gluing
lemma (`toSymplectic_eq_sum_embed_restrictBlock`), the embed-preserves-span
lemma (`toSymplectic_embedBlock_mem_concatSpan`), and the M4 kernel; the *only*
remaining `sorry` is the class-matching concatenated stabilizer
`exists_concatStab_matching_induced`. -/

open NQubitPauliOperator in
/-- `symplecticInner` is additive in the left argument under the group product
(it depends only on the operator part, whose symplectic vector adds, and the
form is bilinear). -/
...
open NQubitPauliOperator in
/-- The identity commutes with everything, so its symplectic inner product is
zero. -/
...
omit [NeZero n₁] in
open NQubitPauliOperator in
/-- Additivity of the block-restricted symplectic inner product: the restriction
of a product multiplies block-wise, so its symplectic inner product against `L`
is additive. -/
...
open NQubitPauliOperator in
/-- The induced class Pauli at block `b`, read off symplectically: its `(x,z)`
bits are the symplectic inner products of the restriction with `Z̄₁` and `X̄₁`.
-/
...
open NQubitPauliOperator in
/-- **(M5, R7 sub-pole.)** The class-matching concatenated stabilizer: given an
outer stabilizer `t` matching the operator part of `inducedOuter D g`, there is
a concatenated stabilizer `u` such that `g * u` restricts, on every block, to an
operator commuting with *both* inner logicals (i.e. inner-stabilizer-like).

`u` is the concat-group product of promoted outer generators in a decomposition
of `t`. The concat-group multiplication — unlike `promoteE t`, which is lossy on
`Y` (`promoteSingle Y = I`) — correctly realizes the `Y = X̄₁Z̄₁` class on
`Y`-blocks. The construction is a `Subgroup.closure_induction` on
`t ∈ Cout.stab` carrying the per-block class-match invariant
`inducedOuterOp D u = t.operators` (base: `promoteE` of a `Y`-free outer
generator, where the class equals the generator's Pauli; multiplicative step:
the inner-logical class is additive under multiplication). Since
`t.operators = inducedOuterOp D g` and the classes match, each
`restrictBlock b (g * u)` has trivial class and so commutes with both inner
logicals. This is the entirety of the remaining M5 work. -/
...
open NQubitPauliOperator in
/-- **(M5.)** Coset injectivity, symplectic form: if an outer stabilizer `t`
matches the operator part of `inducedOuter D g`, then `toSymplectic g.operators`
lies in the concatenated row span (so `g` is concatenated-stabilizer-like).

Assembled from `exists_concatStab_matching_induced` (the class-matching
stabilizer `u`), the gluing lemma, the embed-preserves-span lemma, and the M4
kernel: `g * u` restricts per block to an inner-stabilizer-like operator, so
(gluing + embed-span) `toSymplectic (g * u) ∈ sympSpan`;
`toSymplectic u ∈ sympSpan` since `u` is a concatenated stabilizer; and over
`ZMod 2` `toSymplectic g = toSymplectic (g * u) + toSymplectic u`. -/
...
/-- Coset injectivity: an outer stabilizer matching `inducedOuter D g`'s
operator part forces `g` to be concatenated-stabilizer-like. -/
...
/-- **(M5.)** The induced outer operator of a *nontrivial* concatenated logical
is not an outer stabilizer (else `g` would be concatenated-stabilizer-like,
