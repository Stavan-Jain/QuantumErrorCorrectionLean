...
This file collects small, reusable lemmas about `Subgroup.closure` that are
useful when constructing stabilizer groups:
...
- proving a generated subgroup is abelian from pairwise commutation of
  generators
...
/-- If every generator in `S` commutes with every generator in `T`, then every
element of `closure S` commutes with every element of `closure T`. -/
...
/-- If generators in `S` commute pairwise, then the subgroup `closure S` is
abelian. -/
...
If `S` and `T` commute generatorwise, then every element of `closure (S ∪ T)`
can be written as a product `z * x` with `z ∈ closure S` and `x ∈ closure T`.

This is the key group-theoretic step behind the “CSS decomposition” proofs.
-/

/-- If generators in `S` commute with generators in `T`, then every element of
`closure (S ∪ T)` can be written as `z * x` with `z ∈ closure S` and
`x ∈ closure T`. -/
...
A set S of generators is *independent* if no generator lies in the subgroup
generated by the others. This matches the textbook notion (e.g. Nielsen &
Chuang). The equivalence with symplectic row linear independence is proved
separately (see BinarySymplectic).
-/

/-- A set `S` is an independent generating set if no element of `S` lies in the
subgroup generated by `S` with that element removed. -/
