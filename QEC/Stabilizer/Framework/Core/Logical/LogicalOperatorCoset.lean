...
Logical operators (as mathematical objects) are **cosets of S in the group of
Pauli logical operators** (the centralizer of S). Two Pauli elements represent
the same logical operator iff they lie in the same coset (differ by an element
of S).

For **code distance**, we only count cosets that are "nontrivial": not the
...
constructed here; the element-level API suffices for `HasCodeDistance` and call
sites.
-/
...
/-- If g is in the centralizer and has the same operator part as some s ∈ S,
then g is in the
