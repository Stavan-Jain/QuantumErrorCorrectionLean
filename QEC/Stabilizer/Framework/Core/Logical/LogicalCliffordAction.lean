...
/-- A gate acts as a logical Hadamard on a chosen logical qubit pair `(X̄, Z̄)`
if it is a logical gate and conjugates `X̄` to `Z̄` and `Z̄` to `X̄`.
...
/-- Matrix-bridge formulation of `IsLogicalHadamard` for compatibility with
matrix-level proofs. -/
...
/-- Forgetting the Pauli-action equations, logical Hadamard implies logical
gate. -/
...
/-- Derived logical Y from a chosen logical pair `(X̄, Z̄)`, using the
convention `Ȳ := i X̄ Z̄`. -/
...
/-- A gate acts as a logical phase gate `S` on a chosen logical qubit pair
`(X̄, Z̄)` if it is a logical gate, fixes `Z̄`, and sends `X̄` to
`Ȳ := i X̄ Z̄`. Conjugation convention is `U P U†` (adjoint on the right). -/
...
/-- Matrix-bridge formulation of `IsLogicalS` for compatibility with
matrix-level proofs. -/
...
/-- A gate acts as logical inverse-phase `S†` on a chosen logical qubit pair
`(X̄, Z̄)` if it is a logical gate, fixes `Z̄`, and sends `X̄` to
`-Ȳ = -i X̄ Z̄`. Conjugation convention is `U P U†` (adjoint on the right). -/
...
/-- Matrix-bridge formulation of `IsLogicalSAdjoint` for compatibility with
matrix-level proofs. -/
