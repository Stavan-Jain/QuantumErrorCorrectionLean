...
(Z-part of v)·(X-part of w). The orthogonal of a submodule W is the set of v
with ⟨v,w⟩_s = 0 for all w ∈ W. An operator commutes with all generators iff its
...
/-- Symplectic bilinear form on vectors Fin (n + n) → ZMod 2. Matches the
operator
...
/-- The bilinear form on toSymplectic images equals the operator symplectic
inner. -/
...
/-- The bilinear form is linear in the first argument under scalar
multiplication. -/
...
/-- The bilinear form is linear in the second argument under scalar
multiplication. -/
...
/-- An operator's symplectic vector is in the orthogonal of sympSpan L iff it
commutes
...
/-- If every weight-w operator that commutes with all generators has its
symplectic
