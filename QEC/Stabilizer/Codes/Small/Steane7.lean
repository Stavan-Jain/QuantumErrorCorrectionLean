...
Each pair is a closed identity between two literal group elements, so `decide` settles it:
the kernel multiplies out both sides through the global `DecidableEq (NQubitPauliGroupElement n)`
instance. Mathematically, a Z-row and an X-row meet in an even number of qubits
(`r₁ ∩ r₂ = {0, 1}`, `r₁ ∩ r₃ = {0, 2}`, `r₂ ∩ r₃ = {0, 3}`, and each row with itself in four).
-/
