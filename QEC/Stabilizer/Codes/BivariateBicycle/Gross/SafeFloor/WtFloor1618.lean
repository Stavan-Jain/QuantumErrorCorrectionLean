...
diagonal component-0 datum: each slot's `wt5` is `≡ V₀ (mod 2)` (`slot_parity`),
so the total has the parity of the component-0 weight. This refines
`costFromComps_even` to the per-block level — the achiever-structure lemma (A4
Lemma 28) needs `min_L ≡ min_R ≡ |V₀| (mod 2)` to force the `(min_L, min_R)`
split even, which is what rules out the odd would-be weight-`9` splits in Prop
30. -/
...
/-- **Evenness (Prop 32 ingredient).** Every Smith-coset element
`seamC ζ + ∂₂ f` (with `ζ ∈ ker ∂₂`) has **even** chain weight. Combined with
the `≥ 10` floor (Prop 30) and the `ρ`-link kill (Prop 31), this forces weight
`≥ 12` on the light orbits. Proof: `off_vanish` (Lemma 17) makes component 0 the
diagonal `(V₀, V₀)`, then `costFromComps_even`. -/
...
spine images have sizes `64` (comp 3) and `16` (comp 4); the two reduction
lemmas below pin the exact coordinate change behind those numbers:

* **comp 3** (`spine3_reduce`): `Â₁·t` and `B̂₂·t` of a single `t` share their
  F₄ *direction* `a`, with independent `XY`-shifts — so the `64`-element comp-3
  cell is exactly `(a₃ shared) × (b₃ᴸ, b₃ᴿ free)`.
* **comp 4** (`spine4_reduce`): they share `a` *and* the `B̂₂`-shift is `ω`
  times the `Â₄`-shift (`b₄ᴿ = ω·b₄ᴸ`) — so the `16`-element comp-4 cell is
  `(a₄ shared) × (b₄ᴸ free)`.

Both are foundational convolution facts over the 256-element `Ring`
(`native_decide`, the same category as `rmul_*_mem` / `CRTFrame`'s ideal facts —
*not* the `2³⁰` floor leaf). They feed the reduced-parameter block costs
`blockCostRed{L,R}` below, whose floor `min_L + min_R ≥ 10` (Prop 30) becomes a
finite F₄-knob walk over the `16` spine cells `(a₃, a₄)`. Validated this
session: with this reduction the `Y1` coupled floor is exactly `10` at every one
of the `16` spine cells (matching Prop 30; the over-approximation that frees
`b₄ᴿ` independently spuriously drops it to `8`, confirming the `ω`-linkage is
load-bearing). -/

/-- **comp-3 spine reduction.** The L-side (`Â₁`) and R-side (`B̂₂`) radical
images of any `t` share their slot-frame direction `a`; the `XY`-shifts `b, b'`
are free. `native_decide` over the 256-element `Ring` (foundational, cf.
`rmul_*_mem`). -/
...
/-- **comp-4 spine reduction.** The L-side (`Â₄`) and R-side (`B̂₂`) radical
images of any `t` share their direction `a`, and the `B̂₂`-shift is
`ω·(Â₄-shift)` (`fmul 2`). `native_decide` over the 256-element `Ring`
(foundational). -/
...
/-- The reduced A/left-block cost: the four-slot `mFree2` sum (component 0 =
`V₀` fixed, the unit-side component 2 freed) with comp-1 confining
`a₁·Â₁ + b₁·XY`, spine `a₃·Â₁ + b₃·XY` and `a₄·Â₄ + b₄·XY`, each shifted by the
rep's seam offsets `oL₁, oL₃, oL₄`. The F₄-knob standard form that
`spine3_reduce`/`spine4_reduce` reduce the actual coset L-block to. -/
...
`a₃·B̂₂ + b₃·XY` and `a₄·B̂₂ + b₄·XY`, shifted by `oR₂, oR₃, oR₄`. In the
coupled floor the spine direction `a₃` (and `a₄`) is **shared** with
`blockCostRedL`, and `b₄ = ω·b₄ᴸ` (the `spine4_reduce` linkage). -/
...
weight `chainWeight (seamC ζ + ∂₂ f)` dominates `blockCostRedL + blockCostRedR`
for reduced parameters extracted from `f` — with the spine direction `a₃` (and
`a₄`) **shared** between the two blocks and the comp-4 linkage `b₄ᴿ = ω·b₄ᴸ`,
and a single `F₂`-valued `V₀` shared as component 0. This is the link-free block
bound (`costFromComps_ge_blockLB`) re-expressed through the spine reductions
...
(B/right, frees comp 1) obey the slot-cost rules of Lemma 20 — established here
as small kernel `decide`s over the `2 · 4³ = 128` value tuples (`slot_parity`
scale, axiom-clean). Their empirical shape: with `v₀ = 0` the cost lies in
`{0, 2, 4}` (`0` exactly on the all-zero datum, R1); with `v₀ = 1` it lies in
`{1, 3}`. These feed the per-cell floors `min ≥ 3` and the locus-disjointness
exclusions that the `ge10_of_no_bad_split` skeleton consumes. -/

/-- **R1 (zero slot), A/left.** The `V₀`-fixed L cost vanishes iff every
component of the slot datum is zero. -/
...
/-- **Per-slot parity (Remark 3), A/left.** The `V₀`-fixed L cost is
`≡ v₀ (mod 2)`. -/
...
/-- **Reduced-block parity, A/left.** `blockCostRedL` is `≡ |V₀| (mod 2)` (the
F₂ weight of the diagonal datum) whenever `V₀` is `F₂`-valued: each slot
contributes `≡ V₀ (mod 2)` by `mFree2_parity`. This is the `hpar` input to
...
per-block minima `minL`/`minR` minimize the reduced block cost over the block's
own knobs (the confining `(a₁,b₁)`/`(a₂,b₂)` and the comp-3 shift `b₃`). The
floor `min_L + min_R ≥ 10` (Prop 30) closes the cell — and since
`blockCostRedL ≥ minL` etc. (`minL_le`), the per-cell floor lifts to the full
coset floor. `minL_parity` gives the equal-parity input of the skeleton. -/
...
(`blockCost_parity`: both block minima are `≡ |V₀| (mod 2)`, hence equal mod 2)
and the per-block floors `min ≥ 3` — to **excluding the four "bad splits"**
`(3,3)`, `(3,5)`, `(5,3)`, `(4,4)`. Those exclusions are the locus-disjointness
content of Prop 30 (the `L3=R3=∅` and `L4 ∩ R4 = ∅` facts of A4 §§11.2–11.4),
proved cell by cell. `ge10_of_no_bad_split` is the arithmetic skeleton that
turns the exclusions into the floor; it is pure `omega`. -/

/-- **Remark 5 (cost-8 kill skeleton).** Per-block minima `mL, mR ≥ 3` of equal
parity (`mL ≡ mR (mod 2)`, from slot parity) sum to `≥ 10` once the four bad
splits `(3,3), (3,5), (5,3), (4,4)` are excluded: the equal parity rules out odd
sums, the floor `≥ 3` and the exclusions rule out `6` and `8`. Pure `omega`. -/
...
Prop 31 as hypotheses and discharges `≥ 12` by one `omega` against evenness.
This fixes the proof shape and isolates the two remaining obligations; supplying
them for `Y0, Y1, Y4` (kernel-clean, replacing their `floorOK` leaves) is the
remaining M2/M3 work. Empirically (repo frame, this session): the spine has
`64 × 16 = 1024` cells, so neither Prop 30 nor Prop 31 is a kernel-feasible
enumeration — both need the structured locus argument (A4 §§11.2–11.4 tables +
Lemmas 27–28). -/

/-- **Proposition 32** (light-orbit floor, assembly). For a Smith-coset element
(`ζ ∈ ker ∂₂`), the `≥ 10` floor (Prop 30) and the no-weight-`10` kill (Prop
31), together with evenness (`chainWeight_coset_even`), give weight `≥ 12`. Pure
`omega` glue over the three facts. -/
