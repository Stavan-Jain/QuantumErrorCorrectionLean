...
`∂₁`). Scoped: `open scoped ToricChain`. The `RotatedSurfaceChain` scope binds
the same tokens `Z₁`/`B₁`/`H₁` to the rotated-surface submodules, so open one of
the two scopes per file; the notation names the lattice constants, not the
`ToricCodeN` aliases of them. -/
scoped[ToricChain] notation "Z₁" => Quantum.Stabilizer.Lattice.toricCycles

/-- `B₁ L` is the toric 1-boundary submodule `toricBoundaries L`. Scoped:
`open scoped ToricChain`. -/
...
/-- `H₁ L` is the toric first homology `toricH1 L = Z₁ L ⧸ B₁ L`. Scoped:
`open scoped ToricChain`. -/
