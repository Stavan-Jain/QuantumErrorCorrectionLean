...
/-- `φ : H₁ → (ZMod 2)²` is surjective: every `(h, v)` value is realized by some
cycle. -/
...
/-- `φ : H₁ → (ZMod 2)²` is injective: two cycles with the same `(h, v)`
invariants are homologous. Combined with surjectivity, this yields the
isomorphism `H₁ ≅ (ZMod 2)²`. -/
